...
- **Theme:** The application MUST always support both Light and Dark themes. Ensure proper usage of the standardized CSS variables so that we maintain contrast in icons and fonts across both themes (e.g. use `filter: invert(1)` for icons in dark mode when needed). Default to the high-fidelity Dark Mode palette.
