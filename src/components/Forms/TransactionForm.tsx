    ...
    return (
      <Modal visible={visible} transparent animationType="slide">
        <Pressable style={styles.modalOverlay} onPress={onClose}>
          <View
            style={[styles.modalSheet, { backgroundColor: colors.bg }]}
...
export const TransactionForm: React.FC<Props> = ({ initialData, tags, onSubmit, onDelete }) => {
  ...
  const [activeModal, setActiveModal] = useState<
    'type' | 'date' | 'repeat' | 'tags' | 'until' | null
  >(null);
  ...
  const selectedType = useMemo(
    () => TYPE_OPTIONS.find((opt) => opt.type === formData.type) || TYPE_OPTIONS[1],
    [formData.type],
  );
    ...
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.amountHeader}>
          ...
          <Pressable
            onPress={() => setFormData((prev) => ({ ...prev, amount: 0 }))}
                ...
                <View style={[styles.counter, { borderLeftColor: colors.border }]}>
                  <Pressable
                    onPress={() => {
                      ...
                      setFormData((prev) => ({
                        ...prev,
                        recurringCount: Math.max(2, (prev.recurringCount || 2) - 1),
                      }));
                    }}
                  >
                  ...
                  <Pressable
                    onPress={() => {
                      ...
                      setFormData((prev) => ({
                        ...prev,
                        recurringCount: (prev.recurringCount || 2) + 1,
                      }));
                    }}
                  >
          ...
          <View style={[styles.modalSheet, { backgroundColor: colors.bg }]}>
            ...
            {TYPE_OPTIONS.map((opt) => (
              <Pressable
                ...
                onPress={() => {
                  setFormData((prev) => ({ ...prev, type: opt.type }));
                  setActiveModal(null);
                }}
              >
                ...
                onPress={() => {
                  if (opt.freq === 'none') setFormData((prev) => ({ ...prev, isRecurring: false }));
                    ...
                    setFormData((prev) => ({
                      ...prev,
                      isRecurring: true,
                      recurringFrequency: opt.freq as any,
                    }));
                  setActiveModal(null);
                }}
              >
